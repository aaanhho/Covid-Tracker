...
class menu 
{
    public: // elements declared in this "public" section will be available everywhere // 
		...
		int numPatientDetail; // Initialize variable "numPatientDetail"//
		...
		void readFilePatientDetail(){ // Read patient detail from text file //
			...
			for (int i = 0; i < numPatientDetail; i++) { // for loop //
				...
				PatientDetail temp = PatientDetail (atoi (patientID.c_str()), name, dOB, address, phoneNumber, visitedLocation, dateTime, lastOverseasTravel, listSymptom, covidTest, status);
